...
{
	const static int BDI_SELECTED = 10;
	...
	if (path[row_from][col_from + 1] == 1)
	{
		...
		gmw_move_block(pCGI, row_from - 1, col_from - 1, BDI_SELECTED + arr[row_ori][col_ori], 0, bdi, LEFT_TO_RIGHT, 1);
		disp_move_img(arr, path, row_from, col_from + 1, row_to, col_to, row_ori, col_ori, pCGI, bdi);
	}
	else if (path[row_from + 1][col_from] == 1)
	{
		//disp_move_single(arr, row_from, col_from, row_ori, col_ori, 2, 4, 3);
		gmw_move_block(pCGI, row_from - 1, col_from - 1, BDI_SELECTED + arr[row_ori][col_ori], 0, bdi, UP_TO_DOWN, 1);
		disp_move_img(arr, path, row_from + 1, col_from, row_to, col_to, row_ori, col_ori, pCGI, bdi);
	}
	else if (path[row_from][col_from - 1] == 1)
	{
		//disp_move_single(arr, row_from, col_from, row_ori, col_ori, 3, 4, 3);
		gmw_move_block(pCGI, row_from - 1, col_from - 1, BDI_SELECTED + arr[row_ori][col_ori], 0, bdi, RIGHT_TO_LEFT, 1);
		disp_move_img(arr, path, row_from, col_from - 1, row_to, col_to, row_ori, col_ori, pCGI, bdi);
	}
	else if (path[row_from - 1][col_from] == 1)
	{
		//disp_move_single(arr, row_from, col_from, row_ori, col_ori, 4, 4, 3);
		gmw_move_block(pCGI, row_from - 1, col_from - 1, BDI_SELECTED + arr[row_ori][col_ori], 0, bdi, DOWN_TO_UP, 1);
		disp_move_img(arr, path, row_from - 1, col_from, row_to, col_to, row_ori, col_ori, pCGI, bdi);
	}
}
