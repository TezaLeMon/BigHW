...
{
	int i;
	showstr(x_position + 2, 1 + y_position, "╔", bg_color, fg_color);
	showstr(x_position + 4, 1 + y_position, "═", bg_color, fg_color, col);
	showstr(x_position + 2 * (col + 1) + 2, 1 + y_position, "╗", bg_color, fg_color);
	for (i = 1; i <= row; i++) {
		showstr(x_position + 2, 1 + y_position + i, "║", bg_color, fg_color);
		showstr(x_position + 2 * (col + 1) + 2, 1 + y_position + i, "║", bg_color, fg_color);
	}
	showstr(x_position + 2, 1 + y_position + row + 1, "╚", bg_color, fg_color);
	showstr(x_position + 4, 1 + y_position + row + 1, "═", bg_color, fg_color, col);
	showstr(x_position + 2 * (col + 1) + 2, 1 + y_position + row + 1, "╝", bg_color, fg_color);
	if (is_disp_ch) {
		for (i = 1; i <= row; i++)
			showch(x_position, y_position + 1 + i, 'A' + i - 1);
		for (i = 1; i <= col; i++)
